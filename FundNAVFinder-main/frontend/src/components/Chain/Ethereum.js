...
export default class Ethereum extends Component {
  ...
  async componentDidMount() {
    const balance = await axios.get(`https://api.covalenthq.com/v1/1/address/${this.state.address}/balances_v2/?quote-currency=USD&format=JSON&nft=false&no-nft-fetch=false&key=ckey_ecf46a8674d649acb3cb172f4ad`)
    ...
    balance.data.data.items.map(async (value, index, array) => {
      if (array[index].balance !== "0") {
        ...
        if (data.contractAddress === "0x0ab87046fbb341d058f17cbc4c1133f25a20a52f" || data.contractAddress === "0x04906695d6d12cf5459975d7c3c03356e4ccd460") {
          ohm.push(data)
        }
    ...
    this.setState({ data: table, ohm: ohm })
  }
  ...
  render() {
    return (
      <div className='bg-white my-3'>
        ...
        {this.state.ohm.length !== 0 && (
          <div>
            <div className='font-semibold m-2'>Olympus DAO</div>
            <DataTable value={this.state.ohm} emptyMessage="No Assets">
              <Column field='token' header='Token' body={this.tokenTemplate}/>
              <Column className='text-right' field='balance' header='Quantity' />
              <Column className='text-right' field='price' header='Price' />
              <Column className='text-right' field='value' header='Total Value' />
