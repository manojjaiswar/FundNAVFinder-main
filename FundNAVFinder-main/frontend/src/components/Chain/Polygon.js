...
export default class Polygon extends Component {
  ...
  async componentDidMount() {
    const balance = await axios.get(`https://api.covalenthq.com/v1/137/address/${this.state.address}/balances_v2/?quote-currency=USD&format=JSON&nft=false&no-nft-fetch=false&key=ckey_ecf46a8674d649acb3cb172f4ad`)
    ...
    balance.data.data.items.map(async (value, index, array) => {
      if (array[index].balance !== "0") {
        ...
        if (data.contractAddress === "0xb0c22d8d350c67420f06f48936654f567c73e8c8") {
          klima.push(data)
        }
    ...
    this.setState({ data: table, klima: klima })
  }
  ...
  render() {
    return (
      <div className='bg-white my-3'>
        ...
        {this.state.klima.length !== 0 && (
          <div>
            <div className='font-semibold m-2'>Klima DAO</div>
            <DataTable value={this.state.klima} emptyMessage="No Assets">
              <Column field='token' header='Token' body={this.tokenTemplate}/>
              <Column className='text-right' field='balance' header='Quantity' />
              <Column className='text-right' field='price' header='Price' />
              <Column className='text-right' field='value' header='Total Value' />
